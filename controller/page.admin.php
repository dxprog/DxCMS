...
function renderContent() {
	...
	if ($user === false) {
		...
		if ($user === false) {
			Display::setVariable('content', Display::compile('<admin />', 'admin_login'));
		} else {
...
function newContent($type) {
	...
	if (function_exists('newcontent' . $type)) {
		...
		$render = Display::Compile('<admin />', 'admin_' . $type . '_form');
	}
	Display::setVariable('content', $render);
...
function showContent($type) {
	...
	if (function_exists('showcontent' . $type)) {
		...
		$render = Display::compile($obj, 'admin_' . $type . '_overview');
	}
...
function showContentGallery($type) {
	...
	$render = Display::compile($obj, 'admin_gallery_overview');
	return $render;
...
function editContent($type, $id) {
	...
	if (function_exists('editcontent' . $type)) {
		...
		if (is_numeric($id)) {
			...
			$render = Display::compile($obj, 'admin_' . $type . '_form');
		}
	}
...
function iframeUploadFile() {
	...
	Display::setVariable('upload_info', $retVal);
